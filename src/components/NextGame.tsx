...
export function NextGame({ players, onClear }: NextGameProps) {
  ...
  const [sortedPlayers, setSortedPlayers] = useState<{team1: Player[], team2: Player[]}>({
    team1: [],
    team2: []
  });
  ...
  useEffect(() => {
    if (safePlayers.length === 4) {
      ...
      const isMixedPossible = 
        safePlayers.filter(p => p.gender === "male").length === 2 && 
        safePlayers.filter(p => p.gender === "female").length === 2;
      
      const isLadiesPossible = 
        safePlayers.filter(p => p.gender === "female").length === 4;
